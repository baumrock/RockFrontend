setTimeout(()=>{let isModal=!!document.querySelector("body.modal");if(isModal)return;let reloading=!1;if(window.self!==window.top)return;let redirecttimeout=0;let evtSource;let startStream=function(){let url="./?rockfrontend-livereload="+LiveReloadPage;evtSource=new EventSource(url,{withCredentials:!0});evtSource.onmessage=function(event){let changed=event.data.startsWith("File changed:");if(!changed)return;if(reloading)return;localStorage.setItem("livereload-message",event.data);if(document.hidden){console.log("tab is not visible - waiting for reload");return}
...
let cnt=localStorage.getItem("livereload-count")||0;localStorage.setItem("livereload-count",++cnt);console.log("detected change - reloading "+cnt);reloading=!0;evtSource.close();setTimeout(()=>{document.location.reload(!0)},redirecttimeout)};evtSource.onerror=function(event){if(document.querySelector("#tracy-bs"))return;console.log("Error occurred in EventSource.")};window.addEventListener("beforeunload",()=>{evtSource.close()})};startStream();console.log("RockFrontend is listening for file changes ...")},1000)